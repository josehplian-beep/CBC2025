...
export function brokeredPreviewStorage() {
  ...
  // Post only to the real editor ancestor, validated as an allowed origin, so the
  ...
  const editorOrigins = ancestor && EDITOR_ORIGINS.test(ancestor)
    ? [ancestor]
    : (dev ? ['https://lovable.dev', 'http://localhost:3000'] : ['https://lovable.dev']);
  const RESULT_TYPE = 'lovable-preview-auth:result';
  const TIMEOUT = 2000;
  ...
  const request = (type: string, key: string, value?: string): Promise<{ ok: boolean; value?: string | null } | null> =>
    new Promise((resolve) => {
      ...
      const onMessage = (e: MessageEvent) => {
        ...
        const d = e.data;
        if (d && d.type === RESULT_TYPE && d.requestId === requestId) finish(d);
      };
